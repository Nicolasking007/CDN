...
// 字体颜色
const colorfont = Color.dynamic(Color.black(), Color.white())
// 显示的个数
const maxEvents = previewSize === "large" ? 14 : 5
// 名次栏宽度
const MCsize = 30
// 地区栏宽度 
const DQsize = 90
...
async function createWidget() {
    ...
    if (Olympic_gamesData.data != null) {
        ...
        const events = Olympic_gamesData.data.list
        ...
        if (previewSize == 'large' || previewSize == 'medium') {
            ...
            let _icon = header.addImage(await getImageByUrl(`https://s2.loli.net/2024/07/27/nkXATmosCytuQIj.png`, `ONE-Olympic-logo`, false))
            _icon.imageSize = new Size(35, 40)
            _icon.cornerRadius = 4
            header.addSpacer(10)

            let _title = header.addText(`巴黎奥运会奖牌榜`)
            _title.textColor = colorfont
            ...
            createTextStack(stack, `名次`, MCsize, colorfont)
            createTextStack(stack, ``, imageSize, colorfont)
            createTextStack(stack, `国家/地区`, DQsize, colorfont)
            createTextStack(stack, `🥇金牌`, goldsize, colorfont)
            createTextStack(stack, `🥈银牌`, silversize, colorfont)
            createTextStack(stack, `🥉铜牌`, bronzesize, colorfont)
            createTextStack(stack, `🏅总数`, totalsize, colorfont)
        }
        ...
        for (let i = 0; i < limitedEvents.length; i++) {
            ...
            stack.layoutHorizontally()
            stack.centerAlignContent()
            createTextStack(stack, `${competitors.nocGoldRank}`, MCsize, colorfont)
            ...
            createTextStack(stack, `${competitors.nocName}`, DQsize, colorfont)
            createTextStack(stack, `${competitors.gold}`, goldsize, new Color("#CB8418"))
            createTextStack(stack, `${competitors.silver}`, silversize, new Color("#777777"))
            createTextStack(stack, `${competitors.bronze}`, bronzesize, new Color("#CB6842"))
            createTextStack(stack, `${competitors.total}`, totalsize, colorfont)
            w.addSpacer(2)

        }
    } else {
        w.addSpacer()
        let iconUrl = `https://gips1.baidu.com/it/u=3839228000,1883517164&fm=3028&app=3028&f=PNG&fmt=auto&q=100&size=f360_360`;
        let iconImage = await getImageByUrl(iconUrl, `ONE-Olympic-Thank`, false);
        let iconStack =  w.addStack();
        iconStack.layoutHorizontally();
        iconStack.addSpacer(); 
        let icon = iconStack.addImage(iconImage);
        iconStack.addSpacer(); 
...
async function getOlympicData() {
    ...
    var OlympicData
    try {
        let lolRequest = new Request(atob("aHR0cHM6Ly9hcHAuc3BvcnRzLnFxLmNvbS9tL29seS9tZWRhbHNSYW5rP2NvbXBldGl0aW9uSUQ9MTgwMDAwJmRpc2NpcGxpbmVDb2RlPUFMTCZmcm9tPWg1"));
...
async function updateCheck(localversion) {
    ...
    let uC = versionData
    let originversion = uC['ONE-SuOlympic'].version
    ...
    if (status == 1) {
        ...
        if (!config.runsInWidget) {
            ...
            upd.addAction("立即更新")
            upd.message = uC['ONE-SuOlympic'].notes
            if (await upd.present() == 1) {
                const req = new Request(uC['ONE-SuOlympic'].cdn_scriptURL)
